...
void BFGS::LineSearch()
{
	...
	int m = dimension, n = 1, k = dimension;
	int a = -1, b = 0;
	...
	while (num < 200) 
	{
		...
		//get value of alpha
		Alpha = GetAlpha(Alpha, S_before, g_before);
		...
		T firstItem1 = 1.0 + (*tempOne) * CommonFactor;//1.0 + {[(y)^T * inverseB_before] * y} * CommonFactor 

		//firstItem = [theta * (theta)^T] * CommonFactor * firstItem1 
		m = dimension, n = dimension, k = 1;
		a = 1, b = 0;
		cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans,
			m, n, k, a, theta, k, theta, n, b, temp2Dimension, n);//[theta * (theta)^T] * CommonFactor

		T *firstItem = new T[dimension*dimension];//define firstItem in 2D dimension
		...
		T *secondItem1 = new T[dimension*dimension];//define secondItem1 in 2D dimension
		m = dimension, n = dimension, k = 1;
		a = 1, b = 0;
		cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans,
		...
		T *secondItem2 = new T[dimension*dimension];//define secondItem2 in 2D dimension
		m = dimension, n = 1, k = dimension;
		a = 1, b = 0;
		cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans,
		...
		T *secondItem = new T[dimension*dimension];//define secondItem in 2D dimension
...
T BFGS::GetAlpha(const T &alpha, const T*Sk, const T*gk)
{
	
	EnergyForce BFGS_EF;	

	T newAlpha = alpha;//set old alpha as the new alpha if it does not satisfy the condition
	T *XCart = new T[dimension];//define a dynamic array of new atom coordinates array
	T *XDirect = new T[dimension];//define a dynamic array of new atom coordinates array
	T *gX = new T[dimension];//define a dynamic array of new acceleration array
	...
	//left hand
	T *Cur_lefthand = new T;
	int m = 1, n = 1, k = dimension;
	T a = 1, b = 0;
	cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans,
		m, n, k, a, gX, k, Sk, n, b, Cur_lefthand, n);
	//right hand
	T *Cur_righthand = new T;
	m = 1, n = 1, k = dimension;
	a = Gamma, b = 0;
	cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans,
...
void BFGS::convertDirecttoCartesianCoordinates(const T *coorDirect, T *coorCartesian, const int &N)
{
	...
	for (int i = 0; i < N; i=i+3)
	{
		...
		coorCartesian[i+2] = tempCartesian[2];//x
	}
}
