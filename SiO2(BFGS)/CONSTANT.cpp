...
const std::map<std::string, int> Name_Bonds{ {"Si",4}, {"O", 2}, {"P", 5} };
...
const int cutBondsTimes = 200;//when simulate amorphous system, the number of times to cut bonds.
...
T *latticeVector = new T[3 * 3]; //three lattice vectors defining the unit cell of the system are given
void freeLatticeVector() //free memory 
{
	delete[] latticeVector;
};

T *inverseLatticeVector = new T[3 * 3];//inverse lattice vector
void freeInverseLatticeVector()//free memory
{
	delete[] inverseLatticeVector;
}
