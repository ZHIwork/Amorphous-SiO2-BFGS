...
void Tool::directShortestCoor(const T (&center)[3], const T (&comp)[3], T (&shortest)[3])
{
	...
	{
		shortest[0] = comp[0] - 1;
		//std::cout << "-1: " << shortest[0] << std::endl;
		xcomp = abs_xOsubs1;
...
void Tool::implementMC(bool p)
{
	if (p == 0)
	{
		copyGraph(oldGraph, graph, 64);
		copyCoordinates(atomOldCoordinate, atomCoordinate, totalAtomQuantity * 3);
...
void Tool::copyGraph(const int(&a)[][128], int(&b)[][128], const int &row)
{
	for (int i = 0; i < row; ++i)
	{
		for (int j = 0; j < 128; ++j)
